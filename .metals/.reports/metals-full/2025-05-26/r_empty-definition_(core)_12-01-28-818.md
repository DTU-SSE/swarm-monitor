error id: file://<WORKSPACE>/join-actors/core/src/main/scala/examples/WarehouseMonitor.scala:java/net/DatagramPacket#getOffset().
file://<WORKSPACE>/join-actors/core/src/main/scala/examples/WarehouseMonitor.scala
empty definition using pc, found symbol in pc: java/net/DatagramPacket#getOffset().
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -actor/packet/getOffset.
	 -actor/packet/getOffset#
	 -actor/packet/getOffset().
	 -actor/Result.packet.getOffset.
	 -actor/Result.packet.getOffset#
	 -actor/Result.packet.getOffset().
	 -packet/getOffset.
	 -packet/getOffset#
	 -packet/getOffset().
	 -scala/Predef.packet.getOffset.
	 -scala/Predef.packet.getOffset#
	 -scala/Predef.packet.getOffset().
offset: 5350
uri: file://<WORKSPACE>/join-actors/core/src/main/scala/examples/WarehouseMonitor.scala
text:
```scala
...
//import warehouse_messages.warehouse.{Msg};
...
type Event = warehouse_messages.warehouse.Msg.ValueType

def monitor(algorithm: MatchingAlgorithm) =
  Actor[warehouse_messages.warehouse.Msg.ValueType, Unit] {
    receive { (self: ActorRef[warehouse_messages.warehouse.Msg.ValueType]) =>
      {
...
@tailrec
def receiveLoop(socket: DatagramSocket, monitorFut: Future[Unit], monitorRef: ActorRef[warehouse_messages.warehouse.Msg.ValueType]): Unit = {
    //val bufferSize = 1024
    ...
    val data = java.util.Arrays.copyOfRange(packet.getData, packet.getOffset, packet.get@@Offset + packet.getLength)
    ...
    //val inputStream: InputStream = new ByteArrayInputStream(message.getBytes(StandardCharsets.UTF_8.name()))
    //println(data)

    val parsedMessage = Msg.parseFrom(data)//(inputStream)
    //val parsedMessage = Msg.parseDelimitedFrom(new ByteArrayInputStream(message.getBytes(StandardCharsets.UTF_8.name())))
    //println("HEJ")
    //parsedMessage.containedMessage
    println(parsedMessage.kind.value)
    monitorRef ! parsedMessage.kind.value
