...
* \*\*\* Where `name` is either the field name of the field with type `T1 \| ... \| Tn` or an autogenerated one if `T1 \| ... \| Tn` is used as 'top-level' payload type. None of `T1 \| ... \| Tn` may be literal object types: only primitive types, arrays and type aliases referring to object types are supported.
