...
import myPackage.factory.{EventMessage, Event}
...
object Main:
  ...
  def main(args: Array[String]): Unit =
    ...
    val factoryMonitor = monitor(MatchingAlgorithm.WhileLazyAlgorithm)
    val parse: (Array[Byte]) => Event = (payload: Array[Byte]) => EventMessage.parseFrom(payload).toEvent
    val monitorBridge: MonitorBridge[Event, Unit] = new MonitorBridge(factoryMonitor, parse)
