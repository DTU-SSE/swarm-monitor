...
import { SwarmProtocolType, Subscriptions, Result, InterfacingProtocols, DataResult, overapproxWFSubscriptions, checkComposedSwarmProtocol } from '@actyx/machine-check'
...
// Set forwarder to subscribe to everything
subs_composition['Forwarder'] = ['partReq', 'partOK', 'pos', 'car', 'closingTime',]
...
export const printState = (machineName: string, stateName: string, statePayload: any) => {
  console.log(`${machineName} - State: ${stateName}. Payload: ${statePayload ? JSON.stringify(statePayload, null, 0) : "{}"}`)
}