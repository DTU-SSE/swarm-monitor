...
START_QUALITY_TRANSPORT="npm run start-quality-transport; exec bash"
...
bash split_and_run.sh $1 "$START_STEEL_TRANSPORT" "$START_STAMP" "$START_BODY_ASSEMBLER" \
    ...
    "$START_BASIC_TRANSPORT" "$START_SMART_TRANSPORT" "$START_BASE_STATION" "$START_ENGINE_INSTALLER" "$START_WAREHOUSE" "$START_ENGINE_CHECKER" "$START_WHEEL_INSTALLER" "$START_WHEEL_INSTALLER" "$START_QUALITY_TRANSPORT"
